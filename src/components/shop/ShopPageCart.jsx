...
import { cartRemoveItem, cartUpdateQuantities, cartAction, changeProductQuantityAfterLogin, removeProductAfterLogin } from '../../store/cart';
import { GET_CART_ITEMS, CHANGE_QUANTITY, REMOVE_CART_ITEM} from '../../store/cart/cartActionTypes';
...
class ShopPageCart extends Component {
    ...
    handleChangeQuantity = (item, quantity) => {
        ...
        if(localStorage.getItem('user_token')){
            const { changeProductQuantityAfterLogin } = this.props;
            changeProductQuantityAfterLogin(CHANGE_QUANTITY, item.id, quantity)
        }
    };
    ...
    renderItems() {
        const { cart, cartRemoveItem, removeProductAfterLogin } = this.props;
        return cart.items && cart.items.map((item) => {
                ...
                image = (
                    <div className="product-image">
                        <Link to={url.product(item.product)} className="product-image__body">
                            <img className="product-image__img" src={apiUrlImageProduct + item.product.images[0]} alt="" />
                        </Link>
                    </div>
                );
            }
            ...
            const removeButton = (
                ...
                localStorage.getItem('user_token') ? 
                    <AsyncAction
                        action={() => removeProductAfterLogin(REMOVE_CART_ITEM, item.id)}
                        render={({ run, loading }) => {
            ...
            return (
                <tr key={item.id} className="cart-table__row">
                    ...
                    <td className="cart-table__column cart-table__column--total" data-title="Total">
                        <Currency value={item.total * this.getItemQuantity(item)} />
                    </td>
                        ...
                        <div className="col-12 col-md-7 col-lg-6 col-xl-5">
                            <div className="card">
                                <div className="card-body">
                                    ...
                                    :
                                    <Link to="/account/login" className="btn btn-primary btn-xl btn-block cart__checkout-button">
                                        Login to Proceed
                                    </Link>
                                    }
                                </div>
    ...
    render() {
        ...
        const { cart } = this.props;
        // console.log("my check point in render", cart);
        ...
        if (cart.quantity || cart.items.length > 0) {
            // console.log('thisss',cart);
...
// function checkLoginStatus(state, token) {
//     if (token) return state.cart.cartItems
...
//     // cart:state.cart.cartItems,
//     // console.log(state.cart)
//     return { cart: checkLoginStatus(state, localStorage.getItem('user_token')) }
// };
const mapStateToProps = (state) => (
    // console.log(state.cart)
    {
...
const mapDispatchToProps = {
    ...
    changeProductQuantityAfterLogin,
    removeProductAfterLogin,
};
