...
function ShopPageCategory(props) {
    ...
    useEffect(() => {
        ...
        if (categorySlug) {
            request = shopApi.getCategoryBySlugImplement(categorySlug);
        } else {
    ...
    useEffect(() => {
        ...
        shopApi.getProductsListImplement(
            state.options,
    ...
    useEffect(() => {
        ...
        if (offcanvas === 'always') {
            ...
            shopApi.getLatestProductsImplement({ limit: 5 }).then((result) => {
                if (canceled) {
                    return;
                }
        ...
        <CategorySidebar offcanvas={offcanvas}>
            ...
            {offcanvas !== 'always' && (
                <CategorySidebarItem className="d-none d-lg-block">
                    <WidgetProducts title="Latest Products" path={apiUrlImageProduct} products={latestProducts} />
                </CategorySidebarItem>
            )}
        </CategorySidebar>
    );
