...
import { navItemAction } from '../../store/topnavbar';
import { GET_NAV_ITEMS } from '../../store/topnavbar/navItemActionTypes';

// data stubs
import navLinks from '../../data/headerNavigation';

function NavLinks(props) {
    useEffect(() => props.getNavItems(),[]);
    const { navLink } = props;
    const handleMouseEnter = (event) => {
        const { locale,navLink } = props;
    ...
    const linksList = navLink.items && navLink.items.map((item, index) => {
        let arrow;
        let submenu;
...
const mapStateToProps = (state) => ({
    locale: state.locale,
    navLink:state.navReducer,
});

const mapDispatchToProps = (dispatch) => {
    return {
        getNavItems: () => dispatch(navItemAction(GET_NAV_ITEMS)),
    }
};
