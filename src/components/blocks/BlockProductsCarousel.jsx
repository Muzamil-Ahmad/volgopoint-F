...
export default class BlockProductsCarousel extends Component {
    ...
    productsColumns() {
        ...
        if (rows > 0) {
            // console.log('product slice before', products);
            products = products.slice();
            // console.log('product slice after', products);

            while (products.length > 0) {
                columns.push(products.splice(0, rows));
            }
            // console.log('product slice coument', columns);
        }
    ...
    render() {
        ...
        const columns = this.productsColumns().map((column, index) => {
            const products = column.map((product, key) => (
                <div key={key} className="block-products-carousel__cell">
