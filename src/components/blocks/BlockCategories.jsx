...
export default function BlockCategories(props) {
    ...
    const filteredcategories=categories.filter((item,index)=>((index!=0)?item:''));
    
    const categoriesList = filteredcategories.map((category, index) => {
            ...
            <div key={index} className={classes}>
                <div className=" category-card__body">
                    <div className=" category-card__image">
                        <Link to={category.url}><img src={apiUrlImage+'/'+category.image} alt="" /></Link>
                    </div>
